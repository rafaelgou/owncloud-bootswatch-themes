module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    banner: '/*!\n' +
            ...
            '*/\n',
    swatch: {
      bootstrap:{}, amelia:{}, cerulean:{}, cosmo:{}, cupid:{}, cyborg:{}, flatly:{},
  ...
  grunt.registerTask('build', 'build a regular theme', function(theme, compress) {
    ...
    var files = {};
    var dist = {};
    //concatSrc = 'global/owncloud.less';
    //concatDest = theme + '/build.less';
    lessDest = '<%=builddir%>/' + theme + '/core/css/styles.css';
    lessSrc = [ theme + '/' + 'owncloud.less' ];
...
//    grunt.task.run(['concat', 'less:dist', 'clean:build',
    grunt.task.run(['less:dist', 'clean:build',
      compress ? 'compress:'+lessDest+':'+'<%=builddir%>/' + theme + '/core/css/styles.min.css':'none']);
  });
